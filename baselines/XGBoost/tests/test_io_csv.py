...
from geoplant_xgb.io_csv import build_features_from_meta_and_predictors_pair, load_predictor_pairs


def test_build_features_collapses_long_metadata_and_aligns_one_hot_columns(tmp_path):
    ...
    assert "meta_country_Italy" in test_features.columns
    assert "meta_district_Ain" in train_features.columns
    assert "meta_district_Aosta" in test_features.columns
    assert train_features.loc[0, "clim_bio1"] == 0.1
